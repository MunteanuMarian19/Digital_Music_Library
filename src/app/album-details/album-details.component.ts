...
export class AlbumDetailsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    const albumIndexStr = this.route.snapshot.paramMap.get('albumIndex');

    if (this.artistId && albumIndexStr !== null) {
      this.albumIndex = +albumIndexStr;
      this.artistService.getArtist(this.artistId).subscribe((data) => {
        this.artist = data;
        if (
          this.albumIndex !== null &&
          this.albumIndex >= 0 &&
          this.albumIndex < data.albums.length
        ) {
          this.album = data.albums[this.albumIndex];
        } else {
          console.error('Invalid album index');
        }
      });
    } else {
      console.error('Invalid artistId or albumIndex');
    }
  }
  ...
  addSong(): void {
    ...
    if (this.newSong.title && !isNaN(minutes) && !this.isSongLengthInvalid()) {
      // Combine minutes and seconds into a single length string (e.g., "3:45")
  ...
  updateSong(): void {
    if (
      ...
      this.album.songs[this.editingSongIndex] = { ...this.editingSong };
      this.artistService
        .updateAlbum(this.artistId, this.album._id, this.album)
        .subscribe({
          next: (data) => {
